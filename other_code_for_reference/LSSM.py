...
class LSSM_RT(LSSM):
    """ Wrapper for Real-Time use of LSSM class"""
    def __init__(self, params, output_dim=None, state_dim=None):
        super().__init__(params, output_dim=output_dim, state_dim=state_dim)
        self._system_init()

    def _system_init(self):
        self.xp = np.zeros((self.state_dim, 1))
        self.Pp = np.eye(self.state_dim)
        return self
  

    def kalman_rt(self, Y, x0=None, P0=None, steady_state=True):
        if np.any(np.isnan(self.K)) and steady_state:
            steady_state = False
            print('Steady state Kalman gain not available. Will perform non-steady-state Kalman')
        
        zi = Y[:, np.newaxis] - self.C @ self.xp # Innovation Z(i)
        ...
        if steady_state:
            ...
            ziCov = self.C @ self.Pp @ self.C.T + self.R
            Kf = np.linalg.lstsq(ziCov.T, (self.Pp @ self.C.T).T, rcond=None)[0].T  # Kf(i)
            ...
            P = self.Pp - Kf @ self.C @ self.Pp                   # P(i|i)
        
        if Kf is not None:  # Otherwise cannot do filtering
            X = self.xp + Kf @ zi # X(i|i)
            # allX[i, :] = np.transpose(X)

        newXp = self.A @ self.xp
        newXp += K @ zi

        self.xp = newXp

        if not steady_state:
            self.Pp = self.A @ self.Pp @ self.A.T + self.Q - K @ ziCov @ K.T
