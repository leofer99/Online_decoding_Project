...
def kabsch_algorithm(latents_k, latents_0):
    #2.1 Ensure Day-0 and Day-K have the same number of trials:
    min_trials= min(latents_0.shape[0], latents_k.shape[0])
    ...
    H = Q_centered.T @ P_centered #changed this
...
def apply_rotation(P, R, C_P, C_Q):
    P_centered = P - C_P
    P_rotated = P_centered @ R + C_Q
    return P_rotated
...
def full_process(neural_file_0, neural_file_k, targets_day0_path, targets_dayk_path):
    ...
    R, C_P, C_Q = kabsch_algorithm(latents_k, latents_0)
    latents_k_rotated = apply_rotation(latents_k, R, C_P, C_Q)
    ...
    data_straight_sim_day0, data_right_sim_day0, data_left_sim_day0= separate_by_target(n_trials_k, targets_k, latents_k_rotated)
    ...
    ax.set_title('Kabsch: Comparing Day-K ('+date_dayk+'), Day-0 ('+date_day0+') and Aligned Day-K')
...
# with open(os.path.join(dest_dir, "latents_avg_array_20241108.pkl"), "wb") as f:
#     pickle.dump(avg_latents_0_cut, f)
# with open(os.path.join(dest_dir, "latents_avg_array_20241121.pkl"), "wb") as f:
#     pickle.dump(avg_latents_k_cut, f)
