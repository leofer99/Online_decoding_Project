...
def process_latents(neural_file): #Gets the training latents
...
def process_decoding_latents(neural_file): #Gets the decoding latents (obtained with Kalman filter)

    #load latent variables of training file
    common_path=os.path.dirname(neural_file)
    latents_0_path= os.path.join(common_path, "decoding_latents.pkl")
...
#############################
#   IMPORTANT: In order for this code to work, we need to go to filters.py and comment some lines(they are signaled)
...
neural_filek = neural_file_list[6] 
...
# 2.2 Gets the latents of the training file
latents_0 = process_latents(neural_file0) 
latents_k = process_latents(neural_filek) 
decoding_latents_k = process_decoding_latents(neural_filek) 
...
if len(trialsk)>90:
    ...
    decoding_latents_k=decoding_latents_k[:90]
...
# latents - decoding data (where we apply the CCA)
#############################
...
if split:
    ...
    _, decoding_latents_k_test = train_test_split(decoding_latents_k, test_size=test_size, random_state=random_state)              
    ...
    decoding_latents_k_test=decoding_latents_k
...
latents_sim0_canonical, _ = apply_CCA(CCA_, latents0_test, decoding_latents_k_test)
...
# 8.1 Define model: (Day-0)
model=model0

# 8.2 Define data: (Day-K)
