...
def process_latents(neural_file):
    ...
    #load latent variables of traininf file
...
def CCA_coefficients(latents_0, latents_k, targets_0, targets_k):
    ...
    # avg_straight_0, avg_right_0, avg_left_0= separate_by_target_preprocessing(min_trials, targets_0, latents_0_cut)
    # avg_straight_k, avg_right_k, avg_left_k= separate_by_target_preprocessing(min_trials, targets_k, latents_k_cut)
...
##########################################
# We want to get the latents of decoding files that are calculated with Kalman filtering,
# so that we can use then, for the decoding alignment.
##########################################

CCA={'Offline': False} #keep False
Kalman_latents= False #keep False
...
neural_file0 = neural_file_list[5] #choose the same number for both
...
latents_0 = process_latents(neural_file0) #training file
latents_k = process_latents(neural_filek) #training file
...
# #TRAIN
# CCA_ = CCA_coefficients(latents0_train, latentsk_train, targets_0_train, targets_k_train)
# #TEST
...
Path(dest_dir).mkdir(parents=True, exist_ok=True)
with open(os.path.join(dest_dir, "decoding_latents.pkl"), "wb") as f:
